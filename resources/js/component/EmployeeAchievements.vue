<script setup lang="ts">
interface EmployeeAchievement {
    title: string;
    description: string;
}

interface Employee {
    achievements?: EmployeeAchievement[];
}

const props = defineProps<{
    employee: Employee;
}>();
</script>

<template>
    <!-- Achievements Section -->
    <div class="space-y-4">
        <div class="flex items-center space-x-3">
            <div
                class="w-8 h-8 bg-cyan-100 rounded-lg flex items-center justify-center"
            >
                <svg
                    class="w-4 h-4 text-cyan-600"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                >
                    <path
                        stroke-linecap="round"
                        stroke-linejoin="round"
                        stroke-width="2"
                        d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z"
                    ></path>
                </svg>
            </div>
            <h3 class="text-lg font-semibold text-gray-800">Achievements</h3>
        </div>

        <div class="grid gap-2">
            <template v-if="props.employee.achievements?.length">
                <div
                    v-for="(achievement, i) in props.employee.achievements"
                    :key="i"
                    class="flex items-center space-x-3 p-3 bg-gradient-to-r from-blue-50 to-cyan-50 rounded-lg border border-blue-200/50 hover:shadow-md transition-all duration-200"
                >
                    <div
                        class="w-8 h-8 bg-gradient-to-br from-cyan-400 to-blue-500 rounded-full flex items-center justify-center"
                    >
                        <svg
                            class="w-4 h-4 text-white"
                            fill="currentColor"
                            viewBox="0 0 20 20"
                        >
                            <path
                                d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"
                            ></path>
                        </svg>
                    </div>
                    <div class="flex-1">
                        <p class="font-medium text-gray-800">
                            {{ achievement.title }}
                        </p>
                        <p class="text-sm text-gray-600">
                            {{ achievement.description }}
                        </p>
                    </div>
                </div>
            </template>
            <template v-else>
                <p
                    class="text-gray-400 text-sm italic text-center py-4 bg-gray-50 rounded-lg"
                >
                    No achievements listed
                </p>
            </template>
        </div>
    </div>
</template>
