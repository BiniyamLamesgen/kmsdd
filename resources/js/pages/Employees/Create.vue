...
import { ArrowLeft, Star, Upload } from "lucide-vue-next";
import { z } from "zod";
...
const employeeSchema = z.object({
    ...
    internal_extension: z.string().max(255).optional(),
    ...
    mentoring_interest: z.string().max(5000).optional(),
...
function handleImageChange(e: Event) {
    ...
    if (target.files && target.files[0]) {
        ...
        if (!file.type.startsWith("image/")) {
            ...
            form.profile_picture_file = null;
...
const handleSubmit = () => {
    ...
    // Create FormData exactly like in edit page
    const data = new FormData();
    Object.entries(form.data()).forEach(([key, value]) => {
        if (key === "profile_picture_file" && value instanceof File) {
            data.append("profile_picture", value);
        } else if (
            key !== "profile_picture_file" &&
            key !== "profile_picture" &&
            value !== undefined &&
            value !== null
        ) {
    ...
    form.post(route("employees.store"), data);
};
