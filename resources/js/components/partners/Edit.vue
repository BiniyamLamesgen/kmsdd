<template>
  <div class="container mx-auto px-4 py-8">
    ...
    <div v-else class="max-w-2xl mx-auto bg-white shadow-md rounded-lg p-6">
      <form @submit.prevent="submitForm" enctype="multipart/form-data">
        ...
        <!-- Current Logo -->
        <div v-if="partner.logo && !logoPreview" class="mb-6">
          <label class="block text-sm font-medium text-gray-700 mb-2">Current Logo</label>
          <img :src="partner.logo" :alt="partner.name" class="w-32 h-32 object-cover rounded-md shadow">
        </div>
        ...
        <div class="mb-6">
          ...
          <p class="mt-1 text-sm text-gray-500">
            Upload a new image to replace the current logo. Max size: 2MB.
          </p>
        </div>
  ...
  methods: {
    ...
    async submitForm() {
      try {
        ...
        await axios.post(`/api/partners/${this.partner.id}`, formData, {
          headers: {
        ...
        this.$router.push({
          path: '/partners',
          query: { success: 'Partner updated successfully.' }
        })
      } catch (error) {
